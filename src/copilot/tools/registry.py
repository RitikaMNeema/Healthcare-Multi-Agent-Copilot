...
def invoke_tool(name: str, tool_input: dict, *, role: str, user_id: str, request_id: str, audit, tracing=None) -> object:
    span_ctx = tracing.span(f"tool.{name}", role=role, request_id=request_id) if tracing else nullcontext()
    with span_ctx:
